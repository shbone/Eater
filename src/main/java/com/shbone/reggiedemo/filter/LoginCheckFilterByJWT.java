...
public class LoginCheckFilterByJWT implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        if (check) {
            ...
            UserTokenDTO userTokenDTO = JWTUtil.parseToken(JWTtoken);
            if (redisService.get(userTokenDTO.getUserName()) == null || !redisService.get(userTokenDTO.getUserName()).equals(JWTtoken)) {
                log.error("用户未登录");
                ...
                redisService.set(userTokenDTO.getUserName(), JWTtoken);
                filterChain.doFilter(request, response);
                return;
            }
